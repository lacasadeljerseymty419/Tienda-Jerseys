...
function matchTextBackend(fullText, query) {
  if (!query) return true;
  const normQ = quitarAcentos(query);
  if (!normQ) return true;
  const normT = quitarAcentos(fullText);
  const terms = normQ.split(" ");
  for (let i = 0; i < terms.length; i++) {
    if (terms[i] && normT.indexOf(terms[i]) === -1) return false;
  }
  return true;
}
...
function doPost(e) {
  try {
    ...
    if (action && String(action).indexOf("419") !== -1) {
      // Sin verificación ni creación automática de tablas 419
    }
    ...
    switch (action) {
      ...
      // ==========================================
      // ACCIONES: CONFIGURACIÓN DEL TICKET DE VENTA (80MM)
      // ==========================================
      case "get_ticket_config": {
        return getTicketConfigBackend(ss);
      }

      case "save_ticket_config": {
        return saveTicketConfigBackend(ss, data);
      }
      ...
      case "get_configs": {
        ...
        let cierrePedidosVal = "10:30";
        if (idxCierrePedidos !== -1 && datos.length > 1 && datos[1][idxCierrePedidos] !== "" && datos[1][idxCierrePedidos] !== undefined) {
          const rawValCierre = datos[1][idxCierrePedidos];
          if (rawValCierre instanceof Date) {
            cierrePedidosVal = Utilities.formatDate(rawValCierre, Session.getScriptTimeZone(), "HH:mm");
          } else {
            cierrePedidosVal = String(rawValCierre).trim();
          }
        }
        ...
        const configs = { 
          ...
          estatus_ordenes: [],  
          reglas_envio: [],     
          cierre_pedidos: cierrePedidosVal,
      ...
      case "search": {
        ...
        for (let i = 1; i < datosPlayeras.length; i++) {
          ...
          const rowText = id + " " + nombre + " " + tipo + " " + version + " " + genero;
          if (filtros.nombre && !matchTextBackend(rowText, filtros.nombre)) continue;
      ...
      case "create_client": {
        ...
        // Buscar o crear índices para FechaRegistro y FechaActualizacion
        let idxFechaReg = -1;
        let idxFechaAct = -1;
        for (let c = 0; c < cabeceras.length; c++) {
          const h = String(cabeceras[c]).toLowerCase().replace(/[\s_-]/g, "");
          if (h === "fecharegistro") idxFechaReg = c;
          if (h === "fechaactualizacion") idxFechaAct = c;
        }

        if (idxFechaReg === -1) {
          idxFechaReg = cabeceras.length;
        }
        if (idxFechaAct === -1) {
          idxFechaAct = Math.max(idxFechaReg + 1, cabeceras.length);
        }

        const nowFormatted = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "yyyy-MM-dd HH:mm:ss");
        const fechaReg = data.fecha_registro || nowFormatted;
        const fechaAct = data.fecha_actualizacion || nowFormatted;

        const idCli = "CLI-" + Math.floor(Math.random() * 90000 + 10000);
        const activoVal = (data.activo !== undefined) ? Number(data.activo) : 1;

        const newRow = [idCli, data.nombre_completo, String(data.telefono), String(data.usuario).toLowerCase().trim(), data.password, data.perfil || "Menudeo", data.calle, String(data.numero), data.colonia, data.municipio, String(data.cp), data.referencias, activoVal];

        while (newRow.length <= idxFechaReg) newRow.push("");
        newRow[idxFechaReg] = fechaReg;

        while (newRow.length <= idxFechaAct) newRow.push("");
        newRow[idxFechaAct] = fechaAct;

        hojaClientes.appendRow(newRow);
        return construirRespuesta({ "status": "success", "id_cliente": idCli, "fecha_registro": fechaReg, "fecha_actualizacion": fechaAct });
      }
      ...
      case "update_client": {
        ...
        if (filaEncontrada !== -1) {
          ...
          for (let c = 0; c < cabeceras.length; c++) {
            const h = String(cabeceras[c]).toLowerCase().replace(/[\s_-]/g, "");
            if (h === "fechaactualizacion") {
              idxFechaAct = c;
              break;
            }
          }
          if (idxFechaAct === -1) {
            idxFechaAct = cabeceras.length;
          }
          const nowFormatted = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "yyyy-MM-dd HH:mm:ss");
          const fechaAct = data.fecha_actualizacion || nowFormatted;
          hojaClientes.getRange(filaEncontrada, idxFechaAct + 1).setValue(fechaAct);

          return construirRespuesta({ "status": "success", "message": "Cliente actualizado correctamente.", "fecha_actualizacion": fechaAct });
        } else {
      ...
      case "search_clients": {
        ...
        for (let i = 1; i < datos.length; i++) {
          if (datos[i][0]) {
            ...
            let valFechaReg = (idxFechaReg !== -1 && idxFechaReg < datos[i].length) ? datos[i][idxFechaReg] : "";
            let valFechaAct = (idxFechaAct !== -1 && idxFechaAct < datos[i].length) ? datos[i][idxFechaAct] : "";
            if (valFechaReg instanceof Date) {
              valFechaReg = Utilities.formatDate(valFechaReg, Session.getScriptTimeZone(), "yyyy-MM-dd HH:mm:ss");
            }
            if (valFechaAct instanceof Date) {
              valFechaAct = Utilities.formatDate(valFechaAct, Session.getScriptTimeZone(), "yyyy-MM-dd HH:mm:ss");
            }
            ...
            clientes.push({
              ...
              "fecha_registro": String(valFechaReg || "").trim(),
              "fecha_actualizacion": String(valFechaAct || "").trim()
            });
          }
        }
      ...
      case "save_supplier_order": {
        ...
        if (!sheet) {
          return construirRespuesta({ "status": "error", "message": "No se encontró la pestaña 'PedidosProveedor'." });
        }
      ...
      // ==========================================
      // ACCIÓN: VENTA EXPRÉS / PUNTO DE VENTA LOCAL 419
      // ==========================================
      case "create_pos_419_order": {
        return createPOS419OrderBackend(ss, data);
      }

      // ==========================================
      // ACCIONES: MÓDULO ARTÍCULOS DEPORTIVOS Y DRIVE
      // ==========================================
      case "upload_image_drive": {
        return uploadImageToDriveBackend(ss, data);
      }
      case "setup_tables_articulos": {
        return setupArticulosDeportivosTablas(ss);
      }
      case "search_articulos": {
        return searchArticulosDeportivosBackend(ss, data);
      }
      case "search_articulos_419": {
        return searchArticulosDeportivos419Backend(ss, data);
      }
      case "create_articulo": {
        return createArticuloDeportivoBackend(ss, data);
      }
      case "update_articulo": {
        return updateArticuloDeportivoBackend(ss, data);
      }
      case "update_stock_articulo": {
        return updateStockArticuloBackend(ss, data);
      }
...
function checkAndCreate419Tables(ss) {
  // Desactivada creación y modificación de tablas en Google Sheets
  return;
}
...
function createPOS419OrderBackend(ss, data) {
  try {
    const sheetTallas419 = ss.getSheetByName("Inventario_Tallas419");
    if (!sheetTallas419) {
      return construirRespuesta({ "status": "error", "message": "No se encontró la pestaña 'Inventario_Tallas419'." });
    }

    const items = data.items || [];
    if (!items || items.length === 0) {
      return construirRespuesta({ "status": "error", "message": "No hay artículos en la venta de Punto Venta 419." });
    }

    // 1. Determinar el cliente y tipo de precio (Menudeo vs Mayoreo / Súper Mayoreo)
    let idCliente = String(data.id_cliente || "CLI-MOSTRADOR").trim();
    const nombreClienteRaw = String(data.nombre_cliente || "").trim();
    const perfilAplicado = String(data.tipo_precio_aplicado || "Menudeo").trim();
    const metodoPago = String(data.metodo_pago || "Efectivo").trim();

    if (idCliente === "CLI-MOSTRADOR" || idCliente === "CLI-DESCONOCIDO") {
      if (nombreClienteRaw && nombreClienteRaw !== "Cliente Mostrador") {
        const cleanName = nombreClienteRaw.replace(/^Cliente Mostrador\s*[-–]?\s*/i, "").trim();
        idCliente = "CLI-MOSTRADOR-" + (cleanName || "Cliente Mostrador");
      } else {
        idCliente = "CLI-MOSTRADOR-Cliente Mostrador";
      }
    }

    const esMenudeo = (idCliente.startsWith("CLI-MOSTRADOR") || idCliente === "CLI-DESCONOCIDO" || perfilAplicado.toLowerCase().indexOf("menudeo") !== -1);

    // 2. Seleccionar o crear las tablas destino
    let sheetTargetOrdenes = null;
    let sheetTargetDetalle = null;

    if (esMenudeo) {
      sheetTargetOrdenes = ss.getSheetByName("Ordenes_419") || ss.getSheetByName("Ordenes");
      sheetTargetDetalle = ss.getSheetByName("Ordenes_Detalle_419") || ss.getSheetByName("Ordenes_Detalle");
    } else {
      sheetTargetOrdenes = ss.getSheetByName("Ordenes");
      sheetTargetDetalle = ss.getSheetByName("Ordenes_Detalle");
    }
    if (!sheetTargetOrdenes || !sheetTargetDetalle) {
      return construirRespuesta({ "status": "error", "message": "No se encontraron las tablas de órdenes en Google Sheets." });
    }

    // 3. Validar y descontar stock de Inventario_Tallas419 o Inventario_Articulos419
    const dataTallas419 = sheetTallas419.getDataRange().getValues();
    const sheetArticulos419 = ss.getSheetByName("Inventario_Articulos419");
    const dataArt419 = sheetArticulos419 ? sheetArticulos419.getDataRange().getValues() : [];

    const stockUpdatesTallas = [];
    const stockUpdatesArticulos = [];

    for (let i = 0; i < items.length; i++) {
      const item = items[i];
      const prodIdStr = String(item.id_articulo || item.id_playera || item.id || "").trim().toUpperCase();
      const tallaStr = String(item.talla || item.variante || "").trim().toUpperCase();
      const cantPedida = Number(item.cantidad) || 0;
      const isArticulo = (item.es_articulo === true || prodIdStr.startsWith("ART-"));

      if (isArticulo) {
        if (!sheetArticulos419 || dataArt419.length <= 1) {
          return construirRespuesta({ "status": "error", "message": "No se encontró el inventario de artículos deportivos en Local 419." });
        }

        let filaEncontrada = -1;
        let stockActual = 0;

        for (let a = 1; a < dataArt419.length; a++) {
          const idA = String(dataArt419[a][1]).trim().toUpperCase();
          const varA = String(dataArt419[a][2]).trim().toUpperCase();
          if (idA === prodIdStr && varA === tallaStr) {
            filaEncontrada = a + 1;
            stockActual = Number(dataArt419[a][3]) || 0;
            break;
          }
        }

        if (filaEncontrada === -1 || stockActual < cantPedida) {
          return construirRespuesta({
            "status": "error",
            "message": "Stock insuficiente en Local 419 para el artículo (" + prodIdStr + " - " + tallaStr + "). Disponible: " + stockActual
          });
        }

        stockUpdatesArticulos.push({
          row: filaEncontrada,
          newStock: Math.max(0, stockActual - cantPedida)
        });
      } else {
        let filaEncontrada = -1;
        let stockActual = 0;

        for (let t = 1; t < dataTallas419.length; t++) {
          const idP = String(dataTallas419[t][1]).trim().toUpperCase();
          const sz = String(dataTallas419[t][2]).trim().toUpperCase();
          if (idP === prodIdStr && sz === tallaStr) {
            filaEncontrada = t + 1;
            stockActual = Number(dataTallas419[t][4]) || 0;
            break;
          }
        }
        ...
        if (filaEncontrada === -1 || stockActual < cantPedida) {
          return construirRespuesta({
            "status": "error",
            "message": "Stock insuficiente en Local 419 para la prenda (" + prodIdStr + " - Talla " + tallaStr + "). Disponible: " + stockActual
          });
        }

        stockUpdatesTallas.push({
          row: filaEncontrada,
          newStock: Math.max(0, stockActual - cantPedida)
        });
      }
    ...
    // 4. Ejecutar la deducción de stock en las tablas de inventario 419
    stockUpdatesTallas.forEach(function(u) {
      sheetTallas419.getRange(u.row, 5).setValue(u.newStock);
    });

    if (sheetArticulos419) {
      stockUpdatesArticulos.forEach(function(u) {
        sheetArticulos419.getRange(u.row, 4).setValue(u.newStock);
      });
    }

    // 5. Generar Folio Oficial de Orden POS
    const folioPOS = "ORD-419-" + Math.floor(Math.random() * 900000 + 100000);
    const fechaHoraMX = Utilities.formatDate(new Date(), "America/Mexico_City", "yyyy-MM-dd HH:mm:ss");

    let granTotal = 0;
    items.forEach(function(it) {
      granTotal += (Number(it.subtotal) || (Number(it.precio_unitario || 0) * Number(it.cantidad || 0)));
    });

    // 6. Registrar en la tabla de Ordenes correspondiente (Ordenes_419 u Ordenes)
    sheetTargetOrdenes.appendRow([
      folioPOS,
      idCliente,
      fechaHoraMX,
      granTotal,
      ...
      0,
      granTotal,
      "VENTA-EXPRES-419 (" + metodoPago + ")"
    ]);

    // 7. Registrar en la tabla de Detalle correspondiente (Ordenes_Detalle_419 u Ordenes_Detalle)
    items.forEach(function(it, idx) {
      const idDet = "DET419-" + Math.floor(Math.random() * 900000 + 100000) + "-" + idx;
      const pUnit = Number(it.precio_unitario) || 0;
      const cant = Number(it.cantidad) || 1;
      const subtotalRenglon = Number(it.subtotal) || (pUnit * cant);
      const idPers = it.tipo_personalizacion || it.id_personalizacion || "PERS-NONE";
      const textoPers = it.detalles_personalizacion || it.texto_personalizado || "";
      const prodId = it.id_articulo || it.id_playera || it.id || "Desconocido";
      const tallaStr = String(it.talla || it.variante || "").toUpperCase();

      sheetTargetDetalle.appendRow([
        idDet,
        folioPOS,
        prodId,
        tallaStr,
    ...
    // 8. Si el cliente es de Mayoreo o Súper Mayoreo, procesar acumulación Súper Mayoreo
    let resSuperMayoreo = null;
    if (!esMenudeo && idCliente !== "CLI-MOSTRADOR" && idCliente !== "CLI-DESCONOCIDO") {
      resSuperMayoreo = procesarSuperMayoreoAlCambiarEstatus(ss, folioPOS, "Finalizada en 419");
    }
    ...
    return construirRespuesta({
      ...
      "perfil_aplicado": perfilAplicado,
      "metodo_pago": metodoPago,
      "tabla_ordenes": esMenudeo ? "Ordenes_419" : "Ordenes",
      "tabla_detalle": esMenudeo ? "Ordenes_Detalle_419" : "Ordenes_Detalle",
      "message": "Venta en Local 419 registrada exitosamente en " + (esMenudeo ? "Ordenes_419" : "Ordenes") + ". Orden " + folioPOS,
      "super_mayoreo_procesado": resSuperMayoreo
    });

  } catch (err) {
    Logger.log("Error en createPOS419OrderBackend: " + err.message);
    return construirRespuesta({ "status": "error", "message": "Error al procesar venta en Local 419: " + err.message });
  }
}

// =========================================================================
// MÓDULO DE AUTO-CREACIÓN Y MANEJO DE TABLAS DE ARTÍCULOS DEPORTIVOS
// =========================================================================

function setupArticulosDeportivosTablas(ss) {
  // Desactivada creación automática de tablas en Google Sheets
  return construirRespuesta({ "status": "success", "message": "Validación de creación desactivada." });
}

function searchArticulosDeportivosBackend(ss, data) {
  try {
    setupArticulosDeportivosTablas(ss);
    const sheetArt = ss.getSheetByName("Articulos_Deportivos");
    const sheetInv = ss.getSheetByName("Inventario_Articulos");
    ...
    for (let i = 1; i < dataInv.length; i++) {
      const idA = String(dataInv[i][1]).trim().toUpperCase();
      if (!mapaInv[idA]) mapaInv[idA] = [];
      mapaInv[idA].push({
        id_inventario: dataInv[i][0],
        variante: dataInv[i][2],
        stock: Number(dataInv[i][3]) || 0
      });
    }

    const articulos = [];
    for (let i = 1; i < dataArt.length; i++) {
      const idArt = String(dataArt[i][0] || "").trim();
      if (!idArt) continue;

      const actVal = dataArt[i][7];
      if (actVal !== undefined && actVal !== null && actVal !== "" && Number(actVal) === 0) continue;
      ...
      articulos.push({
        ...
        nombre: dataArt[i][1],
        equipo: dataArt[i][1],
        categoria: dataArt[i][2],
        marca: dataArt[i][3],
        foto: dataArt[i][4],
        precio_menudeo: Number(dataArt[i][5]) || 0,
        precio_mayoreo: Number(dataArt[i][6]) || 0,
        activo: Number(actVal !== undefined ? actVal : 1),
        variantes: mapaInv[idArt.toUpperCase()] || [],
        es_articulo: true
      });
    }

    return construirRespuesta({ "status": "success", "data": articulos });
  } catch (err) {
    return construirRespuesta({ "status": "error", "message": "Error al consultar artículos deportivos: " + err.message });
  }
}

function searchArticulosDeportivos419Backend(ss, data) {
  try {
    setupArticulosDeportivosTablas(ss);
    const sheetArt = ss.getSheetByName("Articulos_Deportivos");
    const sheetInv419 = ss.getSheetByName("Inventario_Articulos419");

    const dataArt = sheetArt.getDataRange().getValues();
    const dataInv419 = sheetInv419.getDataRange().getValues();

    const mapaInv419 = {};
    for (let i = 1; i < dataInv419.length; i++) {
      const idA = String(dataInv419[i][1]).trim().toUpperCase();
      if (!mapaInv419[idA]) mapaInv419[idA] = [];
      mapaInv419[idA].push({
        id_inventario: dataInv419[i][0],
        variante: dataInv419[i][2],
        stock: Number(dataInv419[i][3]) || 0
      });
    }
    ...
    for (let i = 1; i < dataArt.length; i++) {
      ...
      articulos.push({
        ...
        precio_menudeo: Number(dataArt[i][5]) || 0,
        precio_mayoreo: Number(dataArt[i][6]) || 0,
        variantes: mapaInv419[idArt.toUpperCase()] || [],
        es_articulo: true
      });
    }
    ...
    return construirRespuesta({ "status": "error", "message": "Error al consultar existencias 419 de artículos: " + err.message });
  }
}

function createArticuloDeportivoBackend(ss, data) {
  try {
    setupArticulosDeportivosTablas(ss);
    const sheetArt = ss.getSheetByName("Articulos_Deportivos");
    const sheetInv = ss.getSheetByName("Inventario_Articulos");
    ...
    const idArt = "ART-" + Math.floor(Math.random() * 900000 + 100000);
    sheetArt.appendRow([
      idArt,
      data.nombre,
    ...
    variantes.forEach(function(v) {
      const varName = String(v.variante || "Unitalla").trim();
      const stMain = Number(v.stock) || 0;
      const st419 = Number(v.stock_419) || 0;

      const idInv = "INVART-" + Math.floor(Math.random() * 900000 + 100000);
      sheetInv.appendRow([idInv, idArt, varName, stMain]);

      if (st419 > 0) {
        const idInv419 = "INVART419-" + Math.floor(Math.random() * 900000 + 100000);
        sheetInv419.appendRow([idInv419, idArt, varName, st419]);
      }
    });

    SpreadsheetApp.flush();
    return construirRespuesta({ "status": "success", "id_articulo": idArt, "message": "Artículo deportivo " + data.nombre + " creado exitosamente." });
  } catch (err) {
    return construirRespuesta({ "status": "error", "message": "Error al crear artículo deportivo: " + err.message });
  }
}

function updateArticuloDeportivoBackend(ss, data) {
  try {
    setupArticulosDeportivosTablas(ss);
    const sheetArt = ss.getSheetByName("Articulos_Deportivos");
    const dataArt = sheetArt.getDataRange().getValues();

    const idBuscado = String(data.id_articulo || data.id).trim().toUpperCase();
    let rowNum = -1;

    for (let i = 1; i < dataArt.length; i++) {
      if (String(dataArt[i][0]).trim().toUpperCase() === idBuscado) {
        rowNum = i + 1;
        break;
      }
    }
    ...
    if (rowNum === -1) {
      return construirRespuesta({ "status": "error", "message": "No se encontró el artículo con ID: " + idBuscado });
    }

    if (data.nombre) sheetArt.getRange(rowNum, 2).setValue(data.nombre);
    if (data.categoria) sheetArt.getRange(rowNum, 3).setValue(data.categoria);
    if (data.marca) sheetArt.getRange(rowNum, 4).setValue(data.marca);
    if (data.foto !== undefined) sheetArt.getRange(rowNum, 5).setValue(data.foto);
    if (data.precio_menudeo !== undefined) sheetArt.getRange(rowNum, 6).setValue(Number(data.precio_menudeo));
    if (data.precio_mayoreo !== undefined) sheetArt.getRange(rowNum, 7).setValue(Number(data.precio_mayoreo));
    if (data.activo !== undefined) sheetArt.getRange(rowNum, 8).setValue(Number(data.activo));

    SpreadsheetApp.flush();
    return construirRespuesta({ "status": "success", "message": "Artículo " + idBuscado + " actualizado correctamente." });
  } catch (err) {
    return construirRespuesta({ "status": "error", "message": "Error al actualizar artículo deportivo: " + err.message });
  }
}

function updateStockArticuloBackend(ss, data) {
  try {
    setupArticulosDeportivosTablas(ss);
    const is419 = String(data.origen || "").trim() === "419";
    const sheetInv = ss.getSheetByName(is419 ? "Inventario_Articulos419" : "Inventario_Articulos");
    if (!sheetInv) return construirRespuesta({ "status": "error", "message": "Tabla de inventario no encontrada." });

    const dataInv = sheetInv.getDataRange().getValues();
    const idArt = String(data.id_articulo || "").trim().toUpperCase();
    const varTarget = String(data.variante || "").trim().toUpperCase();
    const newStock = Number(data.stock) || 0;

    let rowFound = -1;
    for (let i = 1; i < dataInv.length; i++) {
      if (String(dataInv[i][1]).trim().toUpperCase() === idArt && String(dataInv[i][2]).trim().toUpperCase() === varTarget) {
        rowFound = i + 1;
        break;
      }
    }
    ...
    if (rowFound > 0) {
      sheetInv.getRange(rowFound, 4).setValue(newStock);
    } else {
      const newIdInv = (is419 ? "INVART419-" : "INVART-") + Math.floor(Math.random() * 900000 + 100000);
      sheetInv.appendRow([newIdInv, idArt, data.variante || "Unitalla", newStock]);
    }

    SpreadsheetApp.flush();
    return construirRespuesta({ "status": "success", "message": "Stock de artículo " + idArt + " (" + data.variante + ") actualizado a " + newStock });
  } catch (err) {
    return construirRespuesta({ "status": "error", "message": "Error al actualizar stock de artículo: " + err.message });
  }
}

function uploadImageToDriveBackend(ss, data) {
  try {
    if (!data.base64) {
      return construirRespuesta({ "status": "error", "message": "No se proporcionó la imagen base64." });
    }

    const rawBase64 = String(data.base64);
    ...
    const decodedBytes = Utilities.base64Encode(base64Data);
    const fileName = data.filename || ("foto_articulo_" + new Date().getTime() + ".jpg");
    ...
    if (folders.hasNext()) {
      ...
      const altFolders = DriveApp.getFoldersByName("Fotos_Playeras");
      if (altFolders.hasNext()) {
        folder = altFolders.next();
      } else {
        folder = DriveApp.createFolder("Jersey Store Images");
        folder.setSharing(DriveApp.Access.ANYONE, DriveApp.Permission.VIEW);
      }
    ...
    return construirRespuesta({ "status": "error", "message": "Error al subir imagen a Google Drive: " + err.message });
  }
}

// =========================================================================
// GESTIÓN DIRECTA DE LA HOJA CONFIGURACION_TICKET EN GOOGLE SHEETS
// =========================================================================

function getTicketConfigBackend(ss) {
  try {
    const sheet = ss.getSheetByName("Configuracion_Ticket");
    if (!sheet) {
      return construirRespuesta({ "status": "error", "message": "No se encontró la pestaña 'Configuracion_Ticket'." });
    }
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const v = rows[i][1] !== undefined ? String(rows[i][1]) : '';
      if (k) map[k] = v;
    }

    const cfgObj = {
      nombreTienda: map["ticket_nombre"] || "JERSEYS 419",
      direccion1: map["ticket_dir1"] || "Alamillo # 501, Local 6 (Planta Alta)",
      direccion2: map["ticket_dir2"] || "Bosque Real, Apodaca.",
      horario: map["ticket_horario"] || "Lun a Sáb: 11:00 AM - 8:00 PM | Dom: 12:00 PM - 5:00 PM",
      qrUrl: map["ticket_qr"] || "https://wa.me/528132698182",
      politicas: map["ticket_politicas"] || "",
      mensajePie: map["ticket_pie"] || "*** ¡GRACIAS POR TU COMPRA! ***"
    };

    return construirRespuesta({ "status": "success", "config": cfgObj, "data": cfgObj });
  } catch (err) {
    return construirRespuesta({ "status": "error", "message": "Error al consultar configuración de ticket: " + err.message });
  }
}

function saveTicketConfigBackend(ss, data) {
  try {
    const sheet = ss.getSheetByName("Configuracion_Ticket");
    if (!sheet) {
      return construirRespuesta({ "status": "error", "message": "No se encontró la pestaña 'Configuracion_Ticket'." });
    ...
    const ticketCfg = data.ticket_config || data.config || {};

    const map = {
      "ticket_nombre": ticketCfg.nombreTienda !== undefined ? String(ticketCfg.nombreTienda).trim() : "",
      "ticket_dir1": ticketCfg.direccion1 !== undefined ? String(ticketCfg.direccion1).trim() : "",
      "ticket_dir2": ticketCfg.direccion2 !== undefined ? String(ticketCfg.direccion2).trim() : "",
      "ticket_horario": ticketCfg.horario !== undefined ? String(ticketCfg.horario).trim() : "",
      "ticket_qr": ticketCfg.qrUrl !== undefined ? String(ticketCfg.qrUrl).trim() : "",
      "ticket_politicas": ticketCfg.politicas !== undefined ? String(ticketCfg.politicas).trim() : "",
      "ticket_pie": ticketCfg.mensajePie !== undefined ? String(ticketCfg.mensajePie).trim() : ""
    };

    const rows = sheet.getDataRange().getValues();
    const existingRowIndexes = {};

    for (let i = 1; i < rows.length; i++) {
      const k = String(rows[i][0] || '').trim();
      if (k) existingRowIndexes[k] = i + 1;
    }

    Object.keys(map).forEach(function(k) {
      if (existingRowIndexes[k]) {
        sheet.getRange(existingRowIndexes[k], 2).setValue(map[k]);
      } else {
        sheet.appendRow([k, map[k]]);
      }
    });

    SpreadsheetApp.flush();
    return construirRespuesta({ "status": "success", "message": "Configuración del ticket guardada en Google Sheets exitosamente." });
  } catch (err) {
    return construirRespuesta({ "status": "error", "message": "Error al guardar configuración de ticket: " + err.message });
  }
}
